...
describe('TerritoryAssignmentComponent', () => {
  ...
  beforeEach(async () => {
    mockTerritoryDataService = {
      getNumberTerritory: vi.fn().mockReturnValue(of([{ test: [{ collection: 'coll1', territorio: 1 }] }])),
      getCardTerritorieRegisterTable: vi.fn().mockReturnValue(of([{ id: '1', driver: 'John', applesData: [{ checked: true }] }])),
    ...
    mockActivatedRoute = {
      snapshot: { url: [{ path: 'test', toString: () => 'test' }] }
    };
  ...
  it('should initialize data from sessionStorage if available', () => {
    sessionStorage.setItem('numberTerritory', JSON.stringify({ test: [{ collection: 'coll1', territorio: 1 }] }));
    sessionStorage.setItem('registerStatisticDataTerritorioMT', JSON.stringify([[{ id: '1', driver: 'John', applesData: [{ checked: true }], start: new Date().toISOString() }]]));
  ...
  it('should fetch data if sessionStorage is missing data', () => {
    ...
    expect(mockTerritoryDataService.getCardTerritorieRegisterTable).toHaveBeenCalledWith('coll1');
  });
  ...
  it('should filter data by date correctly', () => {
    ...
    const data = [[
      ...
      { id: '2', driver: 'Old', start: twoYearsAgo.toISOString(), applesData: [{ checked: true }] }
    ]] as any;
  ...
  it('should mark card for delete', () => {
    fixture.detectChanges();
    const card = { id: '1', driver: 'Test' } as any;
  ...
  it('should save all changes', async () => {
    ...
    expect(mockTerritoryDataService.updateCardInCollection).toHaveBeenCalledWith('coll1', '1', { driver: 'Update' });
