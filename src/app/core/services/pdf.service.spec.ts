...
vi.mock('@environments/environment', () => ({
  environment: {
    localities: [
      { key: 'wheelwright', name: 'Wheelwright' }
    ]
  }
}));
...
describe('PdfService', () => {
  ...
  it('should generate PDF', async () => {
    ...
    await service.generateTerritoryAssignmentPDF(s13JPG, territoriesNumber, filterDataListFull, territoryPath);
  ...
  it('should generate PDF with multiple pages if needed', async () => {
    const s13JPG = new ArrayBuffer(8);
    const territoriesNumber = Array.from({ length: 25 }, (_, i) => ({ territorio: i + 1, collection: `w${i + 1}` }));
