...
function findAvailableCongregations() {
  ...
  const congregations = files
    .filter(
      (file) =>
        file.startsWith('environment.') && file.endsWith('.ts') && file !== 'environment.prod.ts',
    )
...
async function main() {
  ...
  if (selectedCongregation === 'new') {
    ...
    if (!config) {
      console.error(
        `❌ Error: No se pudo cargar la configuración de environment.${congregationFileName}.ts`,
      );
      process.exit(1);
    }
    ...
    config.localities.forEach((loc) => {
      console.log(
        `  - ${loc.name} (${loc.territoryPrefix})${!loc.hasNumberedTerritories ? ' [sin numeración]' : ''}`,
      );
    });
    console.log('');
  }
  ...
  for (const localityData of localitiesData) {
    ...
    for (let i = 1; i <= numTerritories; i++) {
      ...
      if (existingDocs.empty) {
        const applesData = Array.from({ length: applesPerTerritory[i - 1] || 0 }, () => ({
          name: '',
          checked: false,
        }));
